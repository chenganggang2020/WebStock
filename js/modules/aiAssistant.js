...
const HANDOFF_PROMPT_STYLES = {
  ...
  'quant-factor': '请切换为“量化因子打分模式”：把趋势、成交额、波动率、相对强弱、板块热度、龙头标签、持仓/自选状态拆成可解释因子，给出因子权重、扣分项和最终排序。',
  valuation: '请切换为“估值安全边际模式”：重点分析估值区间、业绩兑现、成长性、现金流、行业可比公司、估值扩张/压缩风险，并说明需要补充哪些财报数据。',
  'news-sentiment': '请切换为“资讯情绪催化模式”：重点读取近期资讯、公告、政策、研报和市场情绪，区分真实业务催化、概念映射、短期情绪和已经兑现的利好。',
  'event-driven': '请切换为“事件驱动复盘模式”：重点分析公告、财报、订单、政策会议、产业发布会、解禁减持、业绩预告等事件对短中期走势的影响。',
  'macro-sector': '请切换为“宏观行业轮动模式”：从指数环境、流动性、政策方向、行业景气、资金偏好、板块轮动位置判断当前主题是否仍在主线。',
  'liquidity-flow': '请切换为“资金流动性模式”：重点分析成交额、换手、量能持续性、资金承接、缩量/放量、分时强弱和流动性风险。',
  scenario: '请切换为“多空情景推演模式”：分别给出乐观、中性、悲观三种路径，列明触发条件、失败条件、观察价位和需要继续验证的数据。',
  'risk-control': '请切换为“风控止损计划模式”：围绕仓位、回撤、波动、同板块集中暴露、失败信号和复盘动作输出明确的风险控制清单。',
