...
function buildSmartPrompt(result) {
  ...
  const styleGuideMap = {
    ...
    'quant-factor': [
      '重点目标：把候选股拆成量化因子和可解释扣分项。',
      '请按趋势、成交额、相对强弱、波动率、板块热度、龙头标签、持仓/自选状态逐项打分。',
      '输出要给出总分、主要加分因子、主要扣分因子和排序变动原因。'
    ],
    valuation: [
      '重点目标：用估值安全边际视角筛选候选。',
      '请结合主营业务、行业空间、成长性、财报兑现、同行对比和估值扩张/压缩风险。',
      '当前本地数据若缺少 PE、PB、利润率或现金流，请明确列出需要补充的数据。'
    ],
    'news-sentiment': [
      '重点目标：从资讯、公告、政策和市场情绪中识别催化强弱。',
      '请区分真实订单/业绩/政策催化、概念映射、短线情绪和已经充分反映的利好。',
      '输出要说明催化的持续时间、验证指标和退潮信号。'
    ],
    'event-driven': [
      '重点目标：围绕公告、财报、业绩预告、产业会议、政策发布、解禁减持等事件做复盘。',
      '请说明事件对基本面、估值、流动性和短期交易情绪的影响路径。',
      '输出要给出事件确认清单、潜在反转点和需要回避的事件风险。'
    ],
    'macro-sector': [
      '重点目标：从宏观环境和行业轮动位置判断主题是否仍是主线。',
      '请结合指数环境、资金偏好、政策方向、行业景气、板块扩散和高低切换。',
      '输出要区分主线延续、轮动补涨和退潮防守三类候选。'
    ],
    'liquidity-flow': [
      '重点目标：用成交额、换手、量能持续性和分时承接判断可交易性。',
      '请重点分析放量/缩量、资金承接、流动性拥挤、冲高回落和隔日确认条件。',
      '输出要给出流动性充足、需要确认和流动性风险较高的分组。'
    ],
    scenario: [
      '重点目标：用多空情景推演替代单一路径判断。',
      '请分别写出乐观、中性、悲观三种走势路径及触发条件。',
      '输出要包含确认信号、失败信号、观察价位和需要继续验证的数据。'
    ],
    'risk-control': [
      '重点目标：优先识别风险、仓位压力和失败信号。',
      '请结合回撤、波动率、同板块集中暴露、成交萎缩、跌破关键位和交易记录。',
      '输出要给出风险等级、处理优先级和复盘动作清单。'
    ],
    portfolio: [
