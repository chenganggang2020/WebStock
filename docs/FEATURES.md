...
- 人物、书籍与方法研究库支持公开创作者时间线、视频/图片/PDF/笔记归档状态、结构化曲线预览、资料增删、证据分级和语录事件回测；抖音频道可按时间范围检索新增线索、批量导入公开分享链接，并按视频或图文 ID 去重。
- Windows 桌面版提供隔离的持久抖音会话窗口。用户亲自登录后，可主动同步当前已加载页面中的公开作品链接、可见标题、作者、发布时间和页面摘要；身份与频道主页匹配时升级为本人公开来源，不匹配时仍按待核验线索保存。该功能不读取 Cookie、账号密码或浏览器存储，不绕过验证码、反爬和平台权限，也不自动全量下载主页。
