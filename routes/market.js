    ...
    if (Array.isArray(data) && data.length > 0) {
      ...
      for (let i = data.length - 1; i >= 0; i--) {
        ...
        if (item.day && parseFloat(item.close) > 0) {
          lastTradingDay = item.day;
          break;
        }
      }

      if (!lastTradingDay && data.length > 0) {
        const lastItem = data[data.length - 1];
        lastTradingDay = lastItem.day;
      }

      if (lastTradingDay) {
        for (let i = 0; i < data.length; i++) {
          const item = data[i];
          if (item.day && item.day.startsWith(lastTradingDay.split(' ')[0])) {
      ...
      if (lastTradingData.length === 0 && data.length > 0) {
        const lastDay = data[data.length - 1].day.split(' ')[0];
        for (let i = 0; i < data.length; i++) {
          const item = data[i];
          if (item.day && item.day.startsWith(lastDay)) {
            lastTradingData.push({
              time: item.day || '',
              price: parseFloat(item.close) || parseFloat(item.price) || 0,
              volume: parseFloat(item.volume) || 0,
